  ...
  try {
    ...
    if (payload?.method === "getAuraRanking") {
      ...
      for (const endpoint of endpoints) {
        ...
        try {
          const rpcPayload = {
            jsonrpc:"2.0", id:payload.id ?? 1, method:"getProgramAccounts",
          ...
          let data;
          try{ data=JSON.parse(text); }catch{ throw new Error(`Invalid RPC response (HTTP ${response.status})`); }
          ...
          const holders=[...byOwner.entries()]
            ...
            .sort((a,b)=>{
              const ar=BigInt(a.raw), br=BigInt(b.raw);
              return ar>br?-1:ar<br?1:0;
            }).slice(0,100);

          return res.status(200).json({
            jsonrpc:"2.0", id:payload.id ?? 1,
            result:{accountsScanned:(data.result||[]).length,holders}
          });
